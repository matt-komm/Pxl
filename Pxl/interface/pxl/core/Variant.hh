...
#define VARIANT_ADD_TYPE_DECL_POD(NAME, TYPE, VALUE, FIELD) \
	...
	VALUE &as ## NAME() { check(TYPE); return data._u_##FIELD; } \
	const VALUE &as ## NAME() const	{ check(TYPE); return data._u_##FIELD; } \
	static Variant from ## NAME(const VALUE &a) { return Variant(a); } \
	VALUE to ## NAME() const; \
	Variant &operator = (const VALUE &a) { clear(); type = TYPE; data._u_##FIELD = a; return *this; } \
	bool operator != (const VALUE &a) const { check(TYPE); return data._u_##FIELD != a; } \
	bool operator == (const VALUE &a) const { check(TYPE); return data._u_##FIELD == a; } \
	Variant(const VALUE &a) { data._u_##FIELD = a; type = TYPE; }
...
#define VARIANT_ADD_TYPE_DECL_PTR(NAME, TYPE, VALUE, FIELD) \
	...
	Variant &operator =(const VALUE &a) { if (type != TYPE) { clear(); data._u_##FIELD = new VALUE; } type = TYPE; (*data._u_##FIELD) = a; return *this; } \
	Variant(const VALUE &a) { data._u_##FIELD = new VALUE(a); type = TYPE; }
